...
class First(columnName: String) : Aggregator(columnName) {
    ...
    return when (column.type) {
      is StringArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<String?>).first()
        }
      ...
      is IntArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<Int>).first()
        }
      ...
      is DoubleArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<Double>).first()
        }
