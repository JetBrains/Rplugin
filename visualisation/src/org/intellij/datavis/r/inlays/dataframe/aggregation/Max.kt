...
class Max(columnName: String) : Aggregator(columnName) {
    ...
    return when (column.type) {
      is StringArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<String?>).max()!!
        }
      ...
      is IntArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<Int>).max()!!
        }
      ...
      is DoubleArrayType -> {
        ...
        for (i in 0 until column.size) {
          result += (column[i] as ArrayList<Double>).max()!!
        }
