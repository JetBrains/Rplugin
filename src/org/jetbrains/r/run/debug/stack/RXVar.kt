...
internal class RXVar internal constructor(private val rVar: RVar, private val executor: ExecutorService,
                                          private val showHiddenVariables: Boolean = false) : XNamedValue(rVar.name) {
    ...
    executor.execute {
      try {
        ...
        if (rVar.value is RValueEnvironment) {
          addEnvironmentContents(result, vars, executor, showHiddenVariables = showHiddenVariables)
        } else {
  ...
  private fun addListContents(result: XValueChildrenList, vars: List<RVar>) {
    ...
    vars.forEachIndexed { index, it ->
      if (it.name.isEmpty()) {
        ...
        result.add(RXVar(it, executor, showHiddenVariables))
      }
    }
  }
