...
import org.jetbrains.r.visualization.inlays.table.filters.AndFilter
...
abstract class AbstractFiltersHandler : AndFilter(), PropertyChangeListener {
  ...
  var parserModel: org.jetbrains.r.visualization.inlays.table.filters.gui.IParserModel? = null
    set(newParserModel) {
  ...
  protected abstract val choicesHandler: org.jetbrains.r.visualization.inlays.table.filters.gui.ChoicesHandler
  ...
  var autoChoices: org.jetbrains.r.visualization.inlays.table.filters.gui.AutoChoices? = null
    set(mode) {
      if (mode != field) {
        enableNotifications(false)
        field = mode
  ...
  private inner class AutoSelector : RowSorterListener, Runnable, PropertyChangeListener {
    var enabled = org.jetbrains.r.visualization.inlays.table.filters.gui.FilterSettings.autoSelection
      set(enable) {
        if (field != enable) {
          if (enable) {
