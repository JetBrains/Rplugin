...
private fun getEditorBackground() = EditorColorsManager.getInstance().globalScheme.defaultBackground
...
internal open class RNotebookOutputDefaultScrollPane(view: Component, private val validateRoot: Boolean = true) : JBScrollPane(view) {
 ...
 *
 * see [com.intellij.notebooks.visualization.outputs.NotebookOutputNonStickyScrollPane]
 *
 * */
internal open class RNotebookOutputNonStickyScrollPane(
  view: Component,
  validateRoot: Boolean = true,
) : RNotebookOutputDefaultScrollPane(view, validateRoot) {
...
private fun recursivelyRemoveListeners(comp: JComponent) {
  ...
  while (queue.isNotEmpty()) {
    ...
    c.containerListeners.forEach {
      if (it is RNotebookOutputNonStickyScrollPane.MyContainerAdapter) {
        c.removeContainerListener(it)
      }
    }
    c.mouseListeners.forEach {
      if (it is RNotebookOutputNonStickyScrollPane.MyMouseHandler) {
