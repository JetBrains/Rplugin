...
import com.intellij.r.psi.classes.r6.R6ClassActiveBinding
import com.intellij.r.psi.classes.r6.R6ClassField
import com.intellij.r.psi.classes.r6.R6ClassInfo
import com.intellij.r.psi.classes.r6.R6ClassMethod
...
import com.intellij.r.psi.rinterop.AsyncEvent
import com.intellij.r.psi.rinterop.ChunkParameters
import com.intellij.r.psi.rinterop.CommandOutput
import com.intellij.r.psi.rinterop.CommitDataImportRequest
import com.intellij.r.psi.rinterop.ConvertRoxygenToHTMLRequest
import com.intellij.r.psi.rinterop.ConvertRoxygenToHTMLResponse
import com.intellij.r.psi.rinterop.CopyToPersistentRefResponse
import com.intellij.r.psi.rinterop.DataFrameFilterRequest
import com.intellij.r.psi.rinterop.DataFrameGetDataRequest
import com.intellij.r.psi.rinterop.DataFrameGetDataResponse
import com.intellij.r.psi.rinterop.DataFrameInfoResponse
import com.intellij.r.psi.rinterop.DataFrameSortRequest
import com.intellij.r.psi.rinterop.DocumentationForSymbolRequest
import com.intellij.r.psi.rinterop.ExecuteCodeRequest
import com.intellij.r.psi.rinterop.ExecuteCodeResponse
import com.intellij.r.psi.rinterop.GetFunctionSourcePositionResponse
import com.intellij.r.psi.rinterop.GetSysEnvRequest
import com.intellij.r.psi.rinterop.GetVariablesRequest
import com.intellij.r.psi.rinterop.GraphicsGetSnapshotPathRequest
import com.intellij.r.psi.rinterop.GraphicsInitRequest
import com.intellij.r.psi.rinterop.GraphicsRescaleRequest
import com.intellij.r.psi.rinterop.GraphicsRescaleStoredRequest
import com.intellij.r.psi.rinterop.Init
import com.intellij.r.psi.rinterop.LoadEnvironmentRequest
import com.intellij.r.psi.rinterop.ParentEnvsResponse
import com.intellij.r.psi.rinterop.PersistentRefList
import com.intellij.r.psi.rinterop.Plot
import com.intellij.r.psi.rinterop.PreviewDataImportRequest
import com.intellij.r.psi.rinterop.ProtoUtil
import com.intellij.r.psi.rinterop.RIExecutionResult
import com.intellij.r.psi.rinterop.RInstalledPackageList
import com.intellij.r.psi.rinterop.RInterop
import com.intellij.r.psi.rinterop.RInteropCoroutineScope
import com.intellij.r.psi.rinterop.RInteropException
import com.intellij.r.psi.rinterop.RInteropRequestFailed
import com.intellij.r.psi.rinterop.RInteropTerminated
import com.intellij.r.psi.rinterop.RObject
import com.intellij.r.psi.rinterop.RPIServiceGrpc
import com.intellij.r.psi.rinterop.RPersistentRef
import com.intellij.r.psi.rinterop.RRef
import com.intellij.r.psi.rinterop.RRefList
import com.intellij.r.psi.rinterop.RReference
import com.intellij.r.psi.rinterop.RSourceFileManager
import com.intellij.r.psi.rinterop.RepoInstallPackageRequest
import com.intellij.r.psi.rinterop.RepoRemovePackageRequest
import com.intellij.r.psi.rinterop.ScreenParameters
import com.intellij.r.psi.rinterop.SetValueRequest
import com.intellij.r.psi.rinterop.SourcePosition
import com.intellij.r.psi.rinterop.StackFrameList
import com.intellij.r.psi.rinterop.StringList
import com.intellij.r.psi.rinterop.StringOrError
import com.intellij.r.psi.rinterop.TableColumnsInfoRequest
import com.intellij.r.psi.rinterop.UnloadLibraryRequest
import com.intellij.r.psi.rinterop.ValueInfo
import com.intellij.r.psi.rinterop.VariablesResponse
...
class RInteropImpl(
  override val interpreter: RInterpreter, override val processHandler: ProcessHandler,
  address: String, port: Int, override val project: Project,
) : UserDataHolderBase(), Disposable, RInterop {
  ...
  override val globalEnvLoader = globalEnvRef.createVariableLoader()
  val globalEnvEqualityObject = globalEnvRef.getEqualityObject()
  override val currentEnvRef = RReference(RRef.newBuilder().setCurrentEnv(Empty.getDefaultInstance()).build(), this)
  ...
  override val state: RInterpreterState = RInterpreterStateImpl(project, this)
  ...
  override val workingDir: String by Cached("") {
  ...
  override fun executeTask(f: () -> Unit): Promise<Unit> {
  ...
  override fun isLibraryLoaded(name: String): Boolean {
    return loadedPackages.value.keys.contains(name)
  }

  override fun loadLibrary(name: String): CancellablePromise<Unit> {
    return executeAsync(asyncStub::loadLibrary, StringValue.of(name)).thenCancellable {
      invalidateCaches()
    }
  }

  override fun unloadLibrary(name: String, withDynamicLibrary: Boolean): CancellablePromise<Unit> {
  ...
  override fun replExecute(code: String, setLastValue: Boolean, debug: Boolean): CancellablePromise<RIExecutionResult> {
  ...
  override fun httpdRequest(url: String): RInterop.HttpdResponse? {
    return try {
      executeWithCheckCancel(asyncStub::httpdRequest, StringValue.of(url))
        .takeIf { it.success }
        ?.let { RInterop.HttpdResponse(it.content, it.url) }
    }
  ...
  override fun getDocumentationForPackage(packageName: String): CancellablePromise<RInterop.HttpdResponse?> {
    return executeAsync(asyncStub::getDocumentationForPackage, StringValue.of(packageName)).thenCancellable {
      if (it.success) RInterop.HttpdResponse(it.content, it.url) else null
    }
  }

  override fun getDocumentationForSymbol(symbol: String, packageName: String?): CancellablePromise<RInterop.HttpdResponse?> {
  ...
  override fun startHttpd(): Promise<Int> {
  ...
  override fun repoGetPackageVersion(packageName: String): RIExecutionResult {
    return executeRequest(RPIServiceGrpc.getRepoGetPackageVersionMethod(), StringValue.of(packageName))
  }

  override fun repoInstallPackage(packageName: String, fallbackMethod: String?, arguments: Map<String, String>) {
  ...
  override fun dataFrameGetInfo(ref: RReference): CancellablePromise<DataFrameInfoResponse> {
    return executeAsync(asyncStub::dataFrameGetInfo, ref.proto)
  }

  override fun dataFrameGetData(ref: RReference, start: Int, end: Int): CancellablePromise<DataFrameGetDataResponse> {
  ...
  override fun dataFrameSort(ref: RReference, sortKeys: List<RowSorter.SortKey>, disposableParent: Disposable?): RPersistentRef {
  ...
  override fun dataFrameFilter(ref: RReference, f: DataFrameFilterRequest.Filter, disposableParent: Disposable?): RPersistentRef {
  ...
  override fun dataFrameRefresh(ref: RReference): CancellablePromise<Boolean> {
  ...
  override fun convertRoxygenToHTML(functionName: String, functionText: String): RIExecutionResult {
  ...
  override fun getObjectSizes(refs: List<RReference>): List<Long> {
  ...
  private fun processAsyncEvent(event: AsyncEvent) {
    when (event.eventCase) {
      ...
      AsyncEvent.EventCase.SHOWHELPREQUEST -> {
        ...
        val httpdResponse = event.showHelpRequest.takeIf { it.success }?.let { RInterop.HttpdResponse(it.content, it.url) } ?: return
  ...
  override fun <T : Any> cached(defaultValue: T?, f: () -> T): RInterop.Cached<T> {
    return Cached(defaultValue, f)
  }
  inner class Cached<T : Any>(defaultValue: T? = null, val f: () -> T) : RInterop.Cached<T> {
    ...
    override operator fun getValue(thisRef: Any?, property: KProperty<*>): T {
      val currentCacheIndex = this@RInteropImpl.cacheIndex.get()
  ...
  override fun <T> asyncCached(defaultValue: T, f: () -> CancellablePromise<T>): RInterop.AsyncCached<T> {
    return AsyncCached(defaultValue, f)
  }
  inner class AsyncCached<T>(defaultValue: T, private val f: () -> CancellablePromise<T>) : RInterop.AsyncCached<T> {
    ...
    override fun getAsync(): Promise<T> {
      value
  ...
  override val consoleRuntimeInfo: RConsoleRuntimeInfo = RConsoleRuntimeInfoImpl(this)

  override fun copyToPersistentRef(proto: RRef): CancellablePromise<CopyToPersistentRefResponse>
    = executeAsync(asyncStub::copyToPersistentRef, proto)
  override fun loaderGetValueInfo(proto: RRef): CancellablePromise<ValueInfo>
    = executeAsync(asyncStub::loaderGetValueInfo, proto)
  override fun evaluateAsText(proto: RRef): CancellablePromise<StringOrError>
    = executeAsync(asyncStub::evaluateAsText, proto)
  override fun getDistinctStrings(proto: RRef): CancellablePromise<StringList>
    = executeAsync(asyncStub::getDistinctStrings, proto)
  override fun loadObjectNames(proto: RRef): CancellablePromise<StringList>
    = executeAsync(asyncStub::loadObjectNames, proto)
  override fun getEqualityObject(proto: RRef): CancellablePromise<Int64Value>
    = executeAsync(asyncStub::getEqualityObject, proto)
  override fun setValue(request: SetValueRequest): CancellablePromise<ValueInfo>
    = executeAsync(asyncStub::setValue, request)
  override fun disposePersistentRefs(list: PersistentRefList): CancellablePromise<Empty>
    = executeAsync(asyncStub::disposePersistentRefs, list)

  override fun loaderGetParentEnvs(proto: RRef): CancellablePromise<ParentEnvsResponse>
    = executeAsync(asyncStub::loaderGetParentEnvs, proto)
  override fun loaderGetVariables(request: GetVariablesRequest): CancellablePromise<VariablesResponse>
    = executeAsync(asyncStub::loaderGetVariables, request)
