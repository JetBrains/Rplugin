...
object RCondaUtil {
  ...
  private fun getCondaExecutableByName(condaName: String): String? {
    val userHome = StandardFileSystems.local().findFileByPath(SystemProperties.getUserHome().replace('\\', '/'))
    if (userHome != null) {
      for (root in CONDA_DEFAULT_ROOTS) {
        ...
        if (OS.CURRENT == OS.Windows) {
          ...
          if (executableFile != null) return executableFile
          condaFolder = StandardFileSystems.local().findFileByPath("C:\\ProgramData\\$root")
          executableFile = findExecutable(condaName, condaFolder)
          if (executableFile != null) return executableFile
          condaFolder = StandardFileSystems.local().findFileByPath("C:\\$root")
          executableFile = findExecutable(condaName, condaFolder)
          if (executableFile != null) return executableFile
        }
      }
    ...
    if (OS.CURRENT != OS.Windows) {
      val systemCondaFolder = StandardFileSystems.local().findFileByPath("/opt/anaconda")
  ...
  private fun getExecutablePath(homeDirectory: String, name: String): String? {
    ...
    if (runner.exists()) return StandardFileSystems.local().extractPresentableUrl(runner.path)
    ...
    return if (runner.exists()) StandardFileSystems.local().extractPresentableUrl(runner.path) else null
  }
}
