...
import com.intellij.r.psi.rinterop.TableColumnsInfo
...
abstract class TableManipulationAnalyzer<T : TableManipulationFunction> {
   ...
   */
  open fun processTableFromVariable(variable: RIdentifierExpression, processor: Processor<TableColumnInfo>): Boolean {
   ...
   */
  fun processStaticTableColumns(element: RExpression, processor: Processor<TableColumnInfo>): Boolean {
  ...
  private fun getArgumentInfo(function: T,
    ...
    val allArgumentNames = if (function.s3Function) {
      ...
      RPsiUtil.resolveCall((call as? RCallExpression) ?: return null)
        .maxByOrNull { RSkeletonUtilPsi.skeletonFileToRPackage(it.containingFile)?.name == packageName }
  ...
  companion object {
     ...
     */
    private fun getDefaultCountColumnName(operandColumns: Collection<TableColumnInfo>):String {
      var candidate = "n"
      while (true) {
